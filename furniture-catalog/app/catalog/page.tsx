 ...
 <section className="workspace compare-workspace" aria-label="Selected furniture">
  <div className="viewer-area">
   ...
   <div className={'viewer-panes '+(mode==='compare'?'split':'single')}>
    {mode!=='3d'&&<figure className="reference-pane"><div className="pane-label"><ImageIcon size={14}/>{item.source.photo_url?'IKEA catalog photo':'Rendered preview'}</div><div className="reference-image"><CatalogPhoto item={item}/></div><figcaption><span>{item.source.photo_url?'Original product photo · © IKEA':item.source.retailer==='Your uploads'?'Your upload · reconstructed preview':'Homebuddy design · rendered preview'}</span>{item.source.url&&<a href={item.source.url} target="_blank" rel="noreferrer">View product <MoveUpRight size={12}/></a>}</figcaption></figure>}
