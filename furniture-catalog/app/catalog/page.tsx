...
export default function Home(){
 ...
 <section className="workspace compare-workspace" aria-label="Selected furniture">
  <div className="viewer-area">
   <div className="viewer-toolbar"><span className="object-id">IKEA / {item.source.article_number}</span><div className="view-switch" aria-label="View mode"><button aria-pressed={mode==='compare'} onClick={()=>setMode('compare')}><Columns2 size={15}/>Compare</button><button aria-pressed={mode==='photo'} onClick={()=>setMode('photo')}><ImageIcon size={15}/>Catalog photo</button><button aria-pressed={mode==='3d'} onClick={()=>setMode('3d')}><Box size={15}/>3D</button></div></div>
   <div className={'viewer-panes '+(mode==='compare'?'split':'single')}>
    {mode!=='3d'&&<figure className="reference-pane"><div className="pane-label"><ImageIcon size={14}/>IKEA catalog photo</div><div className="reference-image"><CatalogPhoto item={item}/></div><figcaption><span>Original product photo · © IKEA</span><a href={item.source.url} target="_blank" rel="noreferrer">View product <MoveUpRight size={12}/></a></figcaption></figure>}
    {mode!=='photo'&&<div className="recreation-pane"><div className="pane-label"><Box size={14}/>3D recreation</div><div className="model-stage">{!ready||error?<img className="model-poster" src={item.files.preview} alt={item.name+' — rendered preview of unofficial recreation'}/>:createElement('model-viewer',{key:item.id+'-'+revision,ref:viewer,src:item.files.glb,poster:item.files.preview,alt:'Interactive unofficial 3D recreation of '+item.name,'camera-controls':true,'touch-action':'pan-y','shadow-intensity':item.id==='rug'?'0.15':'0.65','shadow-softness':'1',exposure:'1','tone-mapping':'neutral','environment-image':'/environments/catalog-studio-final.hdr','camera-orbit':orbit,'max-camera-orbit':'auto auto 500%','field-of-view':'30deg','interaction-prompt':'none',loading:'eager',reveal:'auto',style:{width:'100%',height:'100%'}})}</div><div className="model-caption"><span role="status">{error?'3D unavailable · Rendered preview':!loaded?'Loading 3D recreation…':'Drag to rotate · Scroll to zoom'}</span>{ready&&!error&&<button className="reset" aria-label="Reset camera view" onClick={()=>setRevision(v=>v+1)}><RotateCcw size={15}/></button>}</div></div>}
   </div>
  </div>
  <aside><p className="eyebrow">{item.category}</p><h2>{item.name}</h2><p className="description">{item.description}</p><a className="retailer-link" href={item.source.url} target="_blank" rel="noreferrer">View original IKEA listing <MoveUpRight size={15}/></a><div className="material-tags">{item.materials.map(d=><span key={d}>{d}</span>)}</div><dl className="dimensions"><div><dt>Width</dt><dd>{cm(dims.width)}<small>cm</small></dd></div><div><dt>Depth</dt><dd>{cm(dims.depth)}<small>cm</small></dd></div><div><dt>Height</dt><dd>{cm(dims.height)}<small>cm</small></dd></div></dl>{item.dimensions_note&&<p className="dimension-note">{item.dimensions_note}</p>}<a className="primary" href={item.files.glb} download><ArrowDownToLine size={17}/>Download recreation <span>GLB</span></a><a className="secondary" href={item.files.blend} download><Layers size={16}/>Editable Blender scene<MoveUpRight size={15}/></a><p className="note">Reconstructed from the linked catalog photo.<br/>Photo and product design belong to IKEA.<br/>Reference checked {item.source.checked_date}.</p></aside>
 </section>
 <section className="browse"><div className="browse-heading"><h3>Furniture for the whole home</h3><span>Original IKEA catalog photos</span></div><div className="object-grid">{catalog.map((obj,i)=><button className={'object-card '+(selected===i?'active':'')} key={obj.id} aria-pressed={selected===i} onClick={()=>select(i)}><div className="thumb"><CatalogPhoto item={obj} thumbnail/><span className="card-index">{String(i+1).padStart(2,'0')}</span><span className="card-open"><MoveUpRight size={15}/></span></div><div className="card-copy"><span>{obj.category}</span><strong>{obj.name}</strong></div></button>)}</div></section>
 <footer><span>FORM / Virtual furniture catalog</span><span>IKEA catalog photography · Independent 3D recreations · Not affiliated with IKEA</span></footer>
 </main>
}
