...
export default function Home(){
 const [catalog,setCatalog]=useState<Item[]>(rawCatalog as Item[]);
 const [catalogError,setCatalogError]=useState('');
 useEffect(()=>{let disposed=false;async function refresh(){try{const response=await fetch('/api/imports?catalog=1',{cache:'no-store'});const data=await response.json() as {items:Item[]};if(!response.ok)throw new Error('Your uploaded assets are temporarily unavailable.');if(disposed)return;setCatalog([...rawCatalog as Item[],...data.items]);setCatalogError('');const id=new URLSearchParams(window.location.search).get('import');if(id){const index=data.items.findIndex((item:Item)=>item.id.startsWith('upload-'+id+'-'));if(index>=0)setSelected(rawCatalog.length+index);}}catch{if(!disposed)setCatalogError('Your uploaded assets are temporarily unavailable.');}}void refresh();window.addEventListener('focus',refresh);return()=>{disposed=true;window.removeEventListener('focus',refresh);};},[]);
