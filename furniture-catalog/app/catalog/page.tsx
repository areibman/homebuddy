...
function CatalogPhoto({item,thumbnail=false}:{item:Item;thumbnail?:boolean}){
 ...
 if(!item.source.photo_url)return <img className={thumbnail?'catalog-thumb':'catalog-photo'} src={item.files.preview} alt={thumbnail?'':item.name+' — rendered preview'} loading={thumbnail?'lazy':'eager'}/>;
 if(failed)return <div className="photo-unavailable"><ImageIcon size={22}/><span>Catalog photo unavailable</span>{!thumbnail&&<a href={item.source.url} target="_blank" rel="noreferrer">View it on {item.source.retailer} ↗</a>}</div>;
 return <img className={thumbnail?'catalog-thumb':'catalog-photo'} src={item.source.photo_url} alt={thumbnail?'':item.name+' — original '+item.source.retailer+' catalog photo'} onError={()=>setFailed(true)} referrerPolicy="no-referrer" loading={thumbnail?'lazy':'eager'}/>;
}
export default function Home(){
 const catalog=useCatalog() as Item[];
 ...
 const viewer=useRef<HTMLElement|null>(null);const item=catalog[selected]||catalog[0];
 ...
 <header><a className="collection-download" href="/"><span className="label-wide">Back to the apartment</span><span className="label-narrow">Back</span></a><a className="collection-download" href="/homes">Homes</a><a className="collection-download" href="/account">Account</a></header>
 ...
 <section className="workspace compare-workspace" aria-label="Selected furniture">
  <div className="viewer-area">
   <div className="viewer-toolbar"><span className="object-id">{item.source.retailer} / {item.source.article_number||'Original design'}</span><div className="view-switch" aria-label="View mode"><button aria-pressed={mode==='compare'} onClick={()=>setMode('compare')}><Columns2 size={15}/>Compare</button><button aria-pressed={mode==='photo'} onClick={()=>setMode('photo')}><ImageIcon size={15}/><span className="label-wide">{item.source.photo_url?'Catalog photo':'Preview'}</span><span className="label-narrow">Photo</span></button><button aria-pressed={mode==='3d'} onClick={()=>setMode('3d')}><Box size={15}/>3D</button></div></div>
   <div className={'viewer-panes '+(mode==='compare'?'split':'single')}>
    {mode!=='3d'&&<figure className="reference-pane"><div className="pane-label"><ImageIcon size={14}/>{item.source.photo_url?item.source.retailer+' catalog photo':'Rendered preview'}</div><div className="reference-image"><CatalogPhoto item={item}/></div><figcaption><span>{item.source.photo_url?'Original product photo · © '+item.source.retailer:item.source.retailer==='Your uploads'?'Your upload · reconstructed preview':'Homebuddy design · rendered preview'}</span>{item.source.url&&<a href={item.source.url} target="_blank" rel="noreferrer">View product <MoveUpRight size={12}/></a>}</figcaption></figure>}
    {mode!=='photo'&&<div className="recreation-pane"><div className="pane-label"><Box size={14}/>3D model</div><div className="model-stage">{!ready||error?<img className="model-poster" src={item.files.preview} alt={item.name+' — rendered model preview'}/>:createElement('model-viewer',{key:item.id+'-'+revision,ref:viewer,src:item.files.glb,poster:item.files.preview,alt:'Interactive 3D model of '+item.name,'camera-controls':true,'touch-action':'pan-y','shadow-intensity':item.id==='rug'?'0.15':'0.65','shadow-softness':'1',exposure:'1','tone-mapping':'neutral','environment-image':studioHdr,'camera-orbit':orbit,'max-camera-orbit':'auto auto 500%','field-of-view':'30deg','interaction-prompt':'none',loading:'eager',reveal:'auto',style:{width:'100%',height:'100%'}})}</div><div className="model-caption"><span role="status">{error?'3D unavailable · Rendered preview':!loaded?'Loading 3D model…':<><span className="hint-fine">Drag to rotate · Scroll to zoom</span><span className="hint-coarse">Drag to rotate · Pinch to zoom</span></>}</span>{ready&&!error&&<button className="reset" aria-label="Reset camera view" onClick={()=>setRevision(v=>v+1)}><RotateCcw size={15}/></button>}</div></div>}
   </div>
  </div>
  <aside><p className="eyebrow">{item.category}</p><h2>{item.name}</h2><p className="description">{item.description}</p>{item.source.url&&<a className="retailer-link" href={item.source.url} target="_blank" rel="noreferrer">View original {item.source.retailer} listing <MoveUpRight size={15}/></a>}<div className="material-tags">{item.materials.map(d=><span key={d}>{d}</span>)}</div><dl className="dimensions"><div><dt>Width</dt><dd>{cm(dims.width)}<small>cm</small></dd></div><div><dt>Depth</dt><dd>{cm(dims.depth)}<small>cm</small></dd></div><div><dt>Height</dt><dd>{cm(dims.height)}<small>cm</small></dd></div></dl>{item.dimensions_note&&<p className="dimension-note">{item.dimensions_note}</p>}<a className="primary" href={item.files.glb} download><ArrowDownToLine size={17}/>Download model <span>GLB</span></a>{item.files.blend&&<a className="secondary" href={item.files.blend} download><Layers size={16}/>Editable Blender scene<MoveUpRight size={15}/></a>}<p className="note">{item.source.photo_url?<>Reconstructed from the linked catalog photo.<br/>Photo and product design belong to {item.source.retailer}.<br/>Reference checked {item.source.checked_date}.</>:<>{item.source.retailer==='Your uploads'?item.provenance:<>Original Homebuddy design.<br/>An editable scene asset, not a retail product.</>}</>}</p></aside>
 </section>
 <section className="browse"><div className="browse-heading"><h3>Furniture & your spaces</h3><span>Catalog pieces, Homebuddy designs & your uploads</span></div><div className="object-grid">{catalog.map((obj,i)=><button className={'object-card '+(selected===i?'active':'')} key={obj.id} aria-pressed={selected===i} onClick={()=>select(i)}><div className="thumb"><CatalogPhoto item={obj} thumbnail/><span className="card-index">{String(i+1).padStart(2,'0')}</span><span className="card-open"><MoveUpRight size={15}/></span></div><div className="card-copy"><span>{obj.category}</span><strong>{obj.name}</strong></div></button>)}</div></section>
 <CustomModels />
<footer><span>FORM / Virtual furniture catalog</span><span>Catalog references · Original Homebuddy designs · Not affiliated with the retailers</span></footer>
 </main>
}
