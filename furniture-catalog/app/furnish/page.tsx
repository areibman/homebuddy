...
function FurnitureDemo({sample,record,home,saved}:{sample:string;record:string;home:HomeDefinition;saved:SavedLayout}){
 ...
 async function arrange(){
  ...
  if(!token)throw new ArrangementError('Sign in from your home before arranging.');
  try{setJob(await call({action:'create',homeId:sample,recordId:record,selection,style:home.plan.design?.name??'Scandinavian'},token));}
 ...
 return <main className="furnish-page">
  ...
  <div className="furnish-body">
   ...
   <aside className="furnish-cart" aria-label="Your furniture selection"><div className="furnish-cart-title"><h2>Your selection</h2><span>{total.count} {total.count===1?'piece':'pieces'}</span></div>
    ...
    <Link className="furnish-existing" href={`/decorate?home=${sample}`}>Walk the current interior <ArrowRight size={14}/></Link>
   </aside></div>
  </div>
 </main>;
}
