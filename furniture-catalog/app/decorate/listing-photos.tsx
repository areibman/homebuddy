...
export function ListingPhotos({home=homeDefinitions['13'],layoutIndex=0}:{home?:HomeDefinition;layoutIndex?:number}){
 ...
 const download=(layout&&home.layoutDownloads?.[layout])||(home.id==='15'&&(!layout||['gather','retreat'].includes(layout))?sampleAssetUrl('/plans/bush-4101-'+(layout??'gather')+'.blend'):home.download);
 ...
 return <div className="listing-gallery" onKeyDown={event=>{if(event.key==='ArrowLeft'||event.key==='ArrowRight'){event.preventDefault();event.stopPropagation();select(index+(event.key==='ArrowRight'?1:-1));}}}>
  <div className="listing-media-tabs" aria-label="Apartment images"><button aria-pressed={index===0} onClick={()=>select(0)}>Floor plan</button>{listing.photos.length>0&&<button aria-pressed={index>0} onClick={()=>select(index||1)}>Listing photos <span>{listing.photos.length}</span></button>}</div><p className="listing-photo-note">{index===0?(!listing.photos.length?listing.note??'Original floor plan.':'Original listing floor plan and dimensions.'):(listing.note??'Spera’s apartment photos. Finishes may vary by unit; these photos are not identified as Plan E.')}</p>
  <div className="listing-photo-stage">
   ...
   {media.length>1&&<><button className="gallery-previous" aria-label="Previous image" onClick={()=>select(index-1)}><ChevronLeft/></button>
