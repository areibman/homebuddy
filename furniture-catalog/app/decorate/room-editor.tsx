...
const money=(id:string,catalog:CatalogItem[])=>{const price=priceFor(id);return price?new Intl.NumberFormat('en-US',{style:'currency',currency:'USD',maximumFractionDigits:price.amount%1===0?0:2}).format(price.amount):catalog.find(item=>item.id===id)?.source.retailer==='Homebuddy'?'Original design':'See IKEA for price';};
...
export function RoomEditor({home=materializeSample('13')!,layoutIndex=0,furnishing}:{home?:HomeDefinition;layoutIndex?:number;furnishing?:{total:string;unpriced:number;onEdit:()=>void;requestLabel?:string}}){
 ...
 return <div className={'decorator '+(mode==='fps'?'is-walking':'')}>
  ...
  <dialog ref={dialog} className={'editor-dialog '+(overlay==='inventory'?'inventory-dialog':overlay==='details'?'details-dialog':'listing-photos-dialog')} aria-labelledby="editor-dialog-title" onCancel={e=>{e.preventDefault();back();}} onClick={e=>{if(e.target===e.currentTarget)back();}} onKeyDown={e=>{if(e.key.toLowerCase()==='e'&&!e.repeat){e.preventDefault();e.stopPropagation();if(overlay==='inventory')close();else if(overlay==='details'){api.current?.cancel();setFromInventory(false);setOverlay('inventory');}}}}>
   ...
   {overlay==='details'&&item&&<><div className="product-detail-body"><div className="product-photo"><CatalogPhoto key={item.id} item={item} link/><span>{item.source.photo_url?'IKEA catalog photo':'Original 3D design · rendered preview'}</span></div><div className="product-copy"><div className="product-brand">{item.source.retailer} <span>{item.source.article_number}</span></div><div className="product-price">{money(item.id,items)}{price&&<small>USD</small>}</div>{price?.note&&<p className="product-note">{price.note}</p>}<dl className="product-dimensions">{(['width','depth','height'] as const).map(key=><div key={key}><dt>{key}</dt><dd>{Math.round(item.dimensions_m[key]*100)}<span> cm</span></dd></div>)}</dl>{item.source.url?<><p className="price-date">{price?'US catalog price · Checked '+price.checked_date+'. Tax and delivery extra.':'Check current pricing on IKEA.'}</p><a className="ikea-product-link" href={item.source.url} target="_blank" rel="noreferrer">View on {item.source.retailer} <ArrowUpRight size={16}/></a></>:<p className="price-date">Original Homebuddy asset. No retail price; ready to place in your room.</p>}<p className="model-note">{item.dimensions_note??'Room models are unofficial 3D recreations.'}</p></div></div><footer><button className="detail-primary" onClick={()=>choose(item.id)}>{detail?.placed?'Place another':'Place in room'}</button>{detail?.placed&&<button className="detail-remove" onClick={()=>{api.current?.remove();close();}}>Remove from room</button>}</footer></>}
