...
import {Box,Footprints,Layers,Scan,SquareArrowOutUpRight,X,ArrowUpRight,PackageOpen,Images,DoorOpen} from 'lucide-react';
...
type Overlay='inventory'|'details'|'plan'|'photos'|null;
...
export function RoomEditor(){
 ...
 useEffect(()=>{let disposed=false;let cleanup:(()=>void)|undefined;
  import('./scene').then(async({mountRoom})=>{if(!host.current||disposed)return;const result=await mountRoom(host.current,{status:setStatus,count:setCount,active:setActive,playing:setPlaying,hint:setHint,motion:setMotion,placement:setValid,door:setDoor,inventory:()=>{setFromInventory(false);setOverlay('inventory');},details:(id:string,placed:boolean)=>{setDetail({id,placed});setFromInventory(false);setOverlay('details');}});if(disposed)result.dispose();else{api.current=result;cleanup=result.dispose;setReady(true);}}).catch(()=>setStatus('The room could not load. Reload to try again.'));
 ...
 return <div className={'decorator '+(mode==='fps'?'is-walking':'')}>
  ...
  <div className="room-layout"><section className="room-stage" aria-label="Apartment editor" aria-busy={!ready||motion!==null}>
   ...
   <div className="view-tools"><div className="camera-toggle" aria-label="Camera view"><button disabled={!ready||motion!==null} aria-pressed={mode==='iso'} onClick={()=>change('iso')}><Layers size={15}/>Isometric</button><button disabled={!ready||motion!==null} aria-pressed={mode==='fps'} onClick={()=>change('fps')}><Footprints size={15}/>Walk</button></div><button className="listing-photos-button" aria-label="Listing photos" disabled={!ready||motion!==null} onClick={()=>{api.current?.setOverlay(true);setOverlay('photos');}}><Images size={17}/><span>Listing photos</span></button><button className="original-plan-button" disabled={!ready||motion!==null} onClick={()=>{api.current?.setOverlay(true);setOverlay('plan');}} aria-label="Original floor plan" title="Original floor plan"><Scan size={18}/></button></div>
   ...
   {active&&<PlacementPopover id={active} hint={hint} valid={valid} place={()=>api.current?.place()} rotate={(direction,pressed)=>api.current?.rotate(direction,pressed)} details={()=>api.current?.details()} remove={()=>api.current?.remove()} cancel={()=>api.current?.cancel()}/>}</>}
  </section></div>
  <dialog ref={dialog} className={'editor-dialog '+(overlay==='inventory'?'inventory-dialog':overlay==='details'?'details-dialog':overlay==='photos'?'listing-photos-dialog':'original-plan-dialog')} aria-labelledby="editor-dialog-title" onCancel={e=>{e.preventDefault();back();}} onClick={e=>{if(e.target===e.currentTarget)back();}} onKeyDown={e=>{if(e.key.toLowerCase()==='e'&&!e.repeat){e.preventDefault();e.stopPropagation();if(overlay==='inventory')close();else if(overlay==='details'){api.current?.cancel();setFromInventory(false);setOverlay('inventory');}}}}>
   <header><div><span className="room-kicker">{overlay==='inventory'?'IKEA COLLECTION':overlay==='details'?'PRODUCT DETAILS':'SPERA · PLAN E'}</span><h1 id="editor-dialog-title">{overlay==='inventory'?'Your inventory':overlay==='details'?item?.name:overlay==='photos'?'Listing photos':'Original floor plan'}</h1></div><button className="dialog-close" autoFocus onClick={back} aria-label={fromInventory?'Back to inventory':'Close dialog'}><X size={19}/><Keycap>esc</Keycap></button></header>
