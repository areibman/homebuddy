...
plan=json.load(open(ROOT/'app/decorate/plan.json'));items={i['id']:i for i in json.load(open(ROOT/'app/decorate/items.json'))}
...
for f in plan['furniture']:
 i=items[f['id']];before=set(bpy.data.objects);bpy.ops.import_scene.gltf(filepath=str(ROOT/'public'/i['files']['glb'].lstrip('/')));objs=set(bpy.data.objects)-before
