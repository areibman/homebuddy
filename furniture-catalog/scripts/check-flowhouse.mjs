...
const p=JSON.parse(fs.readFileSync('app/decorate/homes/flowhouse-wb1/plan.json')),catalog=JSON.parse(fs.readFileSync('app/catalog.json'));
...
assert.equal(p.fixtures.filter(f=>f.type==='toilet').length,2);assert.equal(p.railings.length,3);
console.log(issues);assert.equal(issues.length,0);
console.log('PASS: WB1 layouts fit, all rooms and balcony reachable, both bathrooms and kitchen/laundry complete.');
